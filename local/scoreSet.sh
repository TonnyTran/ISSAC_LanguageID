...
# Build a kaldi score baseline for OLR Challenge 2020
# Zhang Jicheng , July 17 , 2020
...
#####################
default_config=true # if true, all of the data config of lda, submean and whiten are ""$trainset[$trainset $enrollset $testset]""
...
plda_trainset=$trainset # should be one set only
...
mean_diff_scale=1
aplda_trainset=$trainset # should be one set only
...
if [ ! -z $step01 ]; then
    ...
    # lda-submean-whiten-norm-trainplda
    check "$plda_process" "lda submean whiten norm trainplda" plda_process
    # lda-submean-whiten-norm-trainaplda
    ...
    echo "#LOG::Steps 01 Check and generate config Done"
fi
# Determine Your vectortype
if [ ! -z $step02 ]; then
    vectortype=""
    ...
    echo "#LOG Steps02::Determine Your vectortype is $vectortype Done"
fi
...
if [ ! -z $step03 ]; then
    ...
    echo "#LOG Steps03::Configuration information Done"
fi
# Data folder de duplication, check x-vector or I-vector generation error

if [ ! -z $step04 ]; then
    echo "steps4 start"
    echo "Vectordir:"$vectordir
    ...
    echo $allsets | sed 's/ /\n/g' | sed '/^$/d' | sort -u
    # sed '/^$/d',删除空行, sort -u 去重
    for set in $(echo $allsets | sed 's/ /\n/g' | sed '/^$/d' | sort -u); do
        [ ! -d $source_data/$set ] && echo "[exit] No such dir $source_data/$set" && exit 1
        # echo "#############this is do"
        # echo $vectordir/$set
        ...
        echo -e "name $set\ndata $source_data/$set\ndir $vectordir/$set\ninput $vectortype.scp" >$vectordir/$set/config
    done
    echo "#LOG Steps04::Check x-vector or I-vector generation error Done"
fi

if [ ! -z $step05 ]; then
    echo "##Step5 start"
    if [ "$lda" == "true" ]; then
        # 下面的判断中，传入的三个参数 train trian dev
        # lda_data_config="$trainset[$trainset $enrollset $testset]",lda_process="norm-trainlda"
        echo $lda_data_config
        echo $lda_data_config | sed 's/]/\n/g' | sed 's/\[/ /g' | sed '/^$/d' |
            awk -v vdir=$vectordir -v lda_process=$lda_process -v clda=$clda '{
                if($1){
                    print "lda_process",lda_process >> vdir"/"$1"/config";
                }
                for(i=2;i<=NF;i++){
                    if(!a[$i]){
    ...
    # submean_process="lda-getmean"
        echo $submean_data_config | sed 's/]/\n/g' | sed 's/\[/ /g' | sed '/^$/d' |
            ...
            if($1){
                print "submean_process",submean_process >> vdir"/"$1"/config";
            }
            for(i=2;i<=NF;i++){
                if(!a[$i]){
    ...
    # whiten_process="lda-submean-trainwhiten"
        echo $whiten_data_config | sed 's/]/\n/g' | sed 's/\[/ /g' | sed '/^$/d' |
            ...
            if($1){
                print "whiten_process",whiten_process >> vdir"/"$1"/config";
            }
            for(i=2;i<=NF;i++){
                if(!a[$i]){
...
if [ ! -z $step06 ]; then
    ...
    echo "#LOG Steps06::Build connection between score process and enrollset Done"
fi
...
if [ ! -z $step07 ]; then
    ...
    echo "#LOG Steps07::False to speed up and true to clear files with error   Done"
fi
...
if [ ! -z $step08 ]; then
    ...
    echo "#LOG Steps08::Get Trials Done"
fi
...
if [ ! -z $step10 ]; then
    for the_classfier in $(echo $score | sed 's/-/ /g'); do
        ...
        if [ "$the_classfier" == "svm" ]; then
            ...
            enroll_file=$(process $enroll_conf $enroll_process) # enroll_process="mean-lda-submean-whiten-norm"
            writeconf final $enroll_file $enroll_conf
        fi
