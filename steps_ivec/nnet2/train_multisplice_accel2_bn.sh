...
if [ $# != 4 ]; then
  ...
  echo "                                                   # the middle."
  echo "  --bn-layer <bn-layer|6>                          # Hidden layer to put the BN."
  echo "  --bn-dim <bn-dim|80>                          # Dimension of the BN"
...
python2 steps/nnet2/make_multisplice_configs_bn.py contexts --splice-indexes "$splice_indexes" $dir || exit -1;
...
if [ $stage -le -2 ]; then
  ...
  python2 steps/nnet2/make_multisplice_configs_bn.py  \
