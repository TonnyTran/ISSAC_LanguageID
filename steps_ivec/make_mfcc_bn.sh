...
if [ $# -lt 2 ] || [ $# -gt 4 ]; then
   echo "Usage: $0 [options] <data-dir> <nnet_file> [<log-dir> [<mfcc-dir>] ]";
...
for f in $required; do
  if [ ! -f $f ]; then
    echo "make_mfcc_bn.sh: no such file $f"
    exit 1;
  fi
done
...
for n in $(seq $nj); do
  ...
  utils/create_data_link.pl $mfccdir/raw_mfcc_bn_$name.$n.ark
done
...
if [ -f $data/segments ]; then
  ...
  $cmd JOB=1:$nj $logdir/make_mfcc_bn_${name}.JOB.log \
    extract-segments scp,p:$scp $logdir/segments.JOB ark:- \| \
    compute-mfcc-feats $vtln_opts --verbose=2 --config=$mfcc_config ark:- ark:- \| \
    apply-cmvn-sliding --center=true ark:- ark:- \| \
    nnet-am-compute $gpu_opt \
    --chunk-size=${chunk_size} $nnet \
    ark:- ark:- \| \
    copy-feats --compress=$compress $write_num_frames_opt ark:- \
    ark,scp:$mfccdir/raw_mfcc_bn_$name.JOB.ark,$mfccdir/raw_mfcc_bn_$name.JOB.scp \
      || exit 1;
...
if [ -f $logdir/.error.$name ]; then
  ...
  tail $logdir/make_mfcc_bn_${name}.1.log
  exit 1;
fi

# concatenate the .scp files together.
for n in $(seq $nj); do
  cat $mfccdir/raw_mfcc_bn_$name.$n.scp || exit 1;
