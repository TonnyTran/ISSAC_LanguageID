...
if [ $stage -le 0 ]; then
  ...
  if $use_gpu; then
    for g in $(seq $nj); do
      $cmd --gpu 1 ${dir}/log/extract_no_lr.$g.log \
        nnet3-xvector-compute --use-gpu=yes --min-chunk-size=$min_chunk_size --chunk-size=$chunk_size --cache-capacity=${cache_capacity} \
        "$nnet" "`echo $feat | sed s/JOB/$g/g`" ark,scp:${dir}/xvector_no_lr.$g.ark,${dir}/xvector_no_lr.$g.scp || exit 1 &
    done
    wait
  else
    $cmd JOB=1:$nj ${dir}/log/extract_no_lr.JOB.log \
      nnet3-xvector-compute --use-gpu=no --min-chunk-size=$min_chunk_size --chunk-size=$chunk_size --cache-capacity=${cache_capacity} \
      "$nnet" "$feat" ark,scp:${dir}/xvector_no_lr.JOB.ark,${dir}/xvector_no_lr.JOB.scp || exit 1;
  fi
fi

if [ $stage -le 1 ]; then
  echo "$0: combining xvectors across jobs"
  for j in $(seq $nj); do cat $dir/xvector_no_lr.$j.scp; done >$dir/xvector_no_lr.scp || exit 1;
fi

if [ $stage -le 2 ]; then
  # Average the utterance-level xvectors to get speaker-level xvectors.
  echo "$0: computing mean of xvectors for each speaker"
  $cmd $dir/log/speaker_mean_no_lr.log \
    ivector-mean ark:$data/spk2utt scp:$dir/xvector_no_lr.scp \
    ark,scp:$dir/spk_xvector_no_lr.ark,$dir/spk_xvector_no_lr.scp ark,t:$dir/num_utts_no_lr.ark || exit 1;
fi
