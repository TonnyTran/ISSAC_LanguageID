...
function get_trials() {
	...
	#  为了进一步在getTrials中修改trials，通过读取speaker_mean.log文件，可以查看有哪些utt没有Vector，然后就将其从trials中去除
...
function plda() {
	#### $trials $enroll_dir/num_utts.ark $plda $enroll_final $test_final $out_score.score
	the_trials=$1
	num_utt=$2
	plda=$3
	final_enroll=$4
...
function svm() {
	...
	python3 subtools/score/svm/svm_ratelimit.py $final_enroll.svm.data $final_test.svm.data $out_score.tmp \
