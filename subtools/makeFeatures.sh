...
steps/make_${feat_type}${suffix:+_$suffix}${cuda:+_$cuda}.sh $pitch_string --${feat_type}-config $config --nj $nj --cmd "$cmd" $data $exp/${feat_type}/$name/log $exp/${feat_type}/$name || exit 1

echo "Make features done."

if [ $cmvn == "true" ];then
steps/compute_cmvn_stats.sh $data $exp/cmvn/log $exp/cmvn || exit 1
